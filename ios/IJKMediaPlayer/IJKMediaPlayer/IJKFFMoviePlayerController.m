...
// 录制视频（可以录制RTSP/RTMP流）格式mov/mp4，
// streamURL为流地址，filePath录制的文件保存的路径
// _isRecording 来控制开始/停止
- (void)startRecordInPath:(NSString *)localPath {
    if (!_urlString) {
        NSLog(@"开启录制失败，未获取到url");
        return;
    }
    _isRecording = YES;
    //    [self recordVideo:_urlString storageFilePath:localPath];
    const char *filename = [localPath cStringUsingEncoding:NSUTF8StringEncoding];
    ijkmp_record(_mediaPlayer, filename);
